...
export default function History() {
  ...
  const checkpoints = [...new Set(mockEvents.map(e => e.checkpoint))];

  const filtered = mockEvents.filter(e => {
    const matchSearch = search === '' ||
      e.guardName.toLowerCase().includes(search.toLowerCase()) ||
      e.guardId.toLowerCase().includes(search.toLowerCase());
    const matchStatus = filterStatus === 'all' || e.status === filterStatus;
    const matchCP = filterCheckpoint === 'all' || e.checkpoint === filterCheckpoint;
    return matchSearch && matchStatus && matchCP;
  });

  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-white">Patrol History</h1>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={filterCheckpoint} onChange={e => setCP(e.target.value)}
          className="bg-gray-900 border border-gray-700 text-gray-300 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-orange-500">
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800/60">
            {filtered.length === 0 ? (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-gray-600">No events match your filters</td></tr>
            ) : filtered.map(e => (
              <tr key={e.id} className="hover:bg-gray-800/50">
                <td className="px-5 py-3">
                  <div className="text-white font-medium">{e.guardName}</div>
                  <div className="text-gray-600 text-xs font-mono">{e.guardId}</div>
                </td>
                <td className="px-5 py-3 text-gray-300">{e.checkpoint}</td>
                <td className="px-5 py-3 text-gray-400 font-mono">{e.time}</td>
                <td className="px-5 py-3 font-mono text-xs text-gray-400">{e.rssi} dBm</td>
